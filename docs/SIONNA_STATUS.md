...
- Config scaffolding exists (`configs/data_generation_sionna.yaml`) and smoke tests exercise imports and mock mode (`tests/test_sionna_integration.py`).
...
1) Generate scenes with `scripts/generate_scenes.py` (or TileGenerator) → outputs to `data/scenes/` plus `metadata.json` index.  
...
- `configs/data_generation_sionna.yaml`: Sionna-specific knobs (ray counts, antenna config, sampling).
...
- Scene generation (inputs): `src/ue_scene_generation/*`
