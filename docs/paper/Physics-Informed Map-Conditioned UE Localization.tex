...
\begin{itemize}
\item \hlsol{Map-conditioned Transformer posterior.} A dual-encoder architecture (set-based radio Transformer + map ViT) fused by cross-attention.
...
To encode $\mathbf{M}$, we employ a vision Transformer (ViT) backbone \cite{dosovitskiy2021vit} but modify the tokenization stage to induce inductive biases for rotational symmetry.
Specifically, we treat the input map as a geometric field transforming under the trivial representation of the cyclic group $C_4$ (representing $90^\circ$ rotations). We replace the standard linear patch projection with a \emph{strided group-equivariant convolution} (implemented via \texttt{escnn}):
\begin{equation}
  \mathbf{E} = \mathrm{Conv}_{C_4}(\mathbf{M}) \in \R^{d_m \times (H/P) \times (W/P)}.
\end{equation}
By constraining the convolution kernel to satisfy the equivariance condition $\mathcal{K}(g\cdot \mathbf{x}) = \rho_{\text{out}}(g)\mathcal{K}(\mathbf{x})\rho_{\text{in}}(g)^{-1}$ for $g\in C_4$, we ensure that physical rotations of the map explicitly result in corresponding rotations of the feature grid, improving parameter efficiency for urban grids which often exhibit rectilinear symmetries.
The features are then flattened into a sequence of $N=(H/P)\cdot(W/P)$ tokens, summed with 2D sinusoidal positional encodings, and processed by the Transformer encoder to yield the spatial context $\mathbf{S}$.These spatial tokens preserve the map geometry while enabling long-range interactions across the scene.
...
\begin{table}[t]
\centering
\caption{Model architecture and training hyperparameters. We choose a lightweight radio encoder as signal sequences are low-dimensional but require high temporal resolution, while the map encoder is deeper to capture complex spatial geometries. The fusion layer projects map features to the implementation-appropriate latent dimension.}
...
Radio Encoder & \begin{tabular}[t]{@{}l@{}}$d_r=128$, $L=4$, $H=4$\end{tabular} & \textbf{Lightweight:} Radio features are low-dim sets. Set-attention is sufficient to capture temporal/beam correlations. \\
Map Encoder & \begin{tabular}[t]{@{}l@{}}$d_m=256$, $L=4$, $H=4$\\\texttt{patch}=16\end{tabular} & \textbf{Capacity-heavy:} Vision backbone (ViT) needed to parse complex urban geometry (occlusions, canyons). \\
Fusion & \begin{tabular}[t]{@{}l@{}}$d_f=128$, $L=1$, $H=8$\end{tabular} & \textbf{Bottleneck:} Projects rich map context into the signal-focused latent space for efficient querying. \\
Positional Heads & \begin{tabular}[t]{@{}l@{}}Coarse: $512\to G^2$\\Fine: $512\to 256 \to 2K$\end{tabular} & \textbf{Hierarchical:} Deep MLP for coarse ambiguity resolution; shallow head for local Gaussian refinement. \\
\midrule
...
Weights & $\lambda_{\text{phys}}=0.1$ & Regularizes without overriding data evidence. \\
\bottomrule
\end{tabular}
\end{table}
