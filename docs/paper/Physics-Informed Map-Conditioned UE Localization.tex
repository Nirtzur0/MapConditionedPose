...
\node[block, below=0.95cm of x] (re) {Radio encoder\\(Transformer + \texttt{[CLS]})};
\node[block, below=0.95cm of m] (me) {Map encoder\\(ViT / E2-ViT)};
...
\end{tikzpicture}
\vspace{-0.2cm}
\caption{Model overview. A Transformer encoder with \texttt{[CLS]} pooling produces a global radio embedding $\mathbf{z}_r$, a ViT (standard or E2-equivariant) encodes map context into spatial tokens $\mathbf{S}$, multi-query cross-attention yields fused embeddings, and a coarse-to-fine head outputs a top-$K$ Gaussian mixture posterior over 2D UE position.}
