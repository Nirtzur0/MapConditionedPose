...
To encode $\mathbf{M}$, we employ an E(2)-Equivariant Vision Transformer (ViT) architecture that explicitly preserves rotational and reflectional symmetries.
We operate under the discrete group $G=p4m$ (the symmetry group of the square), consisting of 4 rotations and reflections ($|G|=8$).
The input map $\mathbf{M}$ is first mapped from the planar domain to the group domain via a \emph{lifting self-attention} layer:
\begin{equation}
...
Finally, we obtain spatial context tokens $\mathbf{S}$ by flattening the group and spatial dimensions, preserving local equivariant structure for the fusion stage. These spatial tokens enable the model to reason about relative geometry invariant to the absolute orientation of the map.
...
Map Encoder & \begin{tabular}[t]{@{}l@{}}$d_m=256$, $L=4$, $H=4$\\\texttt{group}=$p4m$\end{tabular} & \textbf{Equivariant:} E2-ViT (lifting + group attn) ensures rotation/reflection invariance for urban grids. \\
