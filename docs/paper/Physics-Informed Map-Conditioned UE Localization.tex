...
\node[inputbox, font=\scriptsize] (featsin) at (-2.5, 4.5) {RT/PHY/MAC\\{\tiny 16+8+6=30 dim}};

% Embeddings
\node[embedbox] (embproj) at (-4.8, 3.6) {Embed\\{\tiny →24×3}};
\node[embedbox] (featproj) at (-2.5, 3.6) {Linear\\{\tiny →24×3}};
...
\node[dimtext] at (-1.5, 2.8) {144-dim};

\draw[catarrow] (embproj.south) |- (concat);
\draw[catarrow] (featproj.south) -- (concat);

% Projection
\node[projbox, minimum width=1.6cm, fill=green!30] (inputproj) at (-2.5, 2.1) {Linear→96};
\draw[arrow, green!50!black] (concat) -- (inputproj);

% Transformer
\node[transformer] (radioenc) at (-2.5, 1.0) {\textbf{Transformer}\\{\scriptsize 2 layers, 4 heads, Pre-LN}};
...
% Output
\node[outputbox, fill=orange!25, font=\scriptsize] (radiocls) at (-2.5, -0.1) {$\mathbf{z}_r \in \mathbb{R}^{96}$};
...
% Patch embed
\node[projbox, minimum width=1.6cm, fill=cyan!25] (patchemb) at (3.65, 2.8) {Patch 32×32};
\draw[arrow, cyan!60] (mapconcat) -- (patchemb);

% ViT
\node[transformer, fill=cyan!20] (mapvit) at (3.65, 1.7) {\textbf{ViT (default)}\\{\scriptsize 2 layers, 4 heads}\\{\scriptsize E(2)-ViT optional (p4m)}};
\draw[arrow] (patchemb) -- (mapvit);

% Spatial tokens output
\node[outputbox, fill=cyan!25, font=\scriptsize] (maptokens) at (3.65, 0.5) {$\mathbf{S} \in \mathbb{R}^{64 \times 128}$};
\draw[arrow, cyan!60] (mapvit) -- (maptokens);

% ============================================
% CROSS-ATTENTION FUSION (Center)
% ============================================
\node[fusionbox] (fusion) at (0.5, -1.3) {\textbf{Cross-Attention Fusion}\\{\scriptsize 2 query tokens, 4 heads}};
...
% Fused output
\node[outputbox, fill=violet!25, font=\scriptsize] (fused) at (0.5, -2.4) {$\mathbf{z}_f \in \mathbb{R}^{192}$};
...
% Fine Head  
\node[headbox, fill=yellow!25, font=\scriptsize] (fine) at (2.8, -3.5) {\textbf{Fine Head}\\{\tiny Top-K=3 refinement}};
...
\begin{scope}[on background layer]
    \node[draw=blue!40, dashed, rounded corners=6pt, fit=(embin)(radioenc)(radiocls), inner sep=6pt, fill=blue!3] {};
    \node[draw=cyan!40, dashed, rounded corners=6pt, fit=(radiomap)(osmmap)(mapvit)(maptokens), inner sep=6pt, fill=cyan!3] {};
\end{scope}

% Feature annotations
\node[font=\tiny, text=gray, align=left, anchor=west] at (5.8, 4.3) {Radio map: RSRP, RSRQ,\\SINR, CQI, Throughput};
\node[font=\tiny, text=gray, align=left, anchor=west] at (5.8, 3.4) {OSM: Height, Footprint,\\Roads, Terrain, Material};

\end{tikzpicture}
\vspace{-0.2cm}
\caption{\textbf{Model architecture.} The \textcolor{blue!70!black}{Radio Encoder} tokenizes multi-layer measurements (RT, PHY, MAC features including CSI reports and Doppler descriptors) via learned embeddings and projections, processes tokens with a 2-layer Transformer, and extracts a global embedding $\mathbf{z}_r$ via a [CLS] token. The \textcolor{cyan!70!black}{Map Encoder} fuses radio and OSM maps (10 channels total) through patch embedding and a standard ViT by default (optional E(2)-equivariant variant), producing spatial tokens $\mathbf{S}$. \textcolor{violet!70!black}{Cross-attention fusion} attends radio queries to map keys/values with 2 learnable query tokens. The \textcolor{red!70!black}{Coarse Head} predicts a 32×32 cell probability distribution, and the \textcolor{yellow!50!orange}{Fine Head} outputs sub-cell offsets and scores for the Top-K cells, yielding a re-ranked \textcolor{teal!70!black}{hypothesis distribution} over 2D position.}
...
\end{equation}
where $\Vert$ denotes concatenation, $e_c,e_b$ are learned embeddings, $P$ projects continuous features (RT, PHY, MAC measurements), and $\psi$ is a time embedding. The feature vector $\mathbf{f}_t$ includes CSI reports (CQI/PMI/RI) that reflect channel quality and spatial structure, plus Doppler descriptors (Doppler spread and coherence time) that capture time-variation statistics rather than raw CSI/CFR tensors.
...
\paragraph{Implementation note: CSI reports and Doppler.}
Our current implementation uses standardized CSI reports (CQI/PMI/RI) and Doppler statistics as scalar features. Doppler is represented indirectly via summary statistics (spread and coherence time), not by passing raw CSI/CFR tensors through the encoder.
...
Max sequence & $T=11$ & 2\,s window at 200\,ms report interval; balances temporal coverage and memory. \\
\midrule
\multicolumn{3}{l}{\textit{Neural Architecture}} \\
Radio Encoder & \begin{tabular}[t]{@{}l@{}}$d_r=96$, $L=2$, $H=4$\\\texttt{[CLS]} pooling\end{tabular} & \textbf{Standard Transformer:} Processes radio token sets with self-attention. \texttt{[CLS]} token provides permutation-invariant pooling. \\
Map Encoder & \begin{tabular}[t]{@{}l@{}}\textbf{Default:} Standard ViT\\$d_m=128$, $L=2$, $H=4$\\\texttt{patch\_size}=$32$\end{tabular} & \textbf{Efficient:} Standard ViT is computationally efficient and performs well when map orientations are consistent. \\
...
Fusion & \begin{tabular}[t]{@{}l@{}}$d_f=192$, $H=4$\\$K_q=2$ query tokens\end{tabular} & \textbf{Multi-query:} Multiple learnable queries attend to map tokens, conditioned on radio features. \\
...
Top-$K$ size & Top-$K=3$ & Retains dominant ambiguity modes while reducing compute. \\
Loss weights & $\lambda_{\text{coarse}}=1.5$, $\lambda_{\text{fine}}=1.0$ & Fine expected error is primary; coarse ensures correct cell is in top-$K$. \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Input map stack used by the map encoder. The top row shows the five radio-map channels (RSRP, RSRQ, SINR, CQI, throughput), and the bottom row shows the semantic channels (height/footprint/roads/terrain).}
\label{fig:map_layers}
\end{figure}

Figure~\ref{fig:ue_trajectories} shows example UE trajectories over the map extent; trajectories are sampled at a fixed report interval and encoded as temporal measurement sequences. Figure~\ref{fig:feature_violin} summarizes the per-dimension distributions of RT/PHY/MAC features used by the radio encoder.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{figures/ue_trajectories.png}
\caption{Sampled UE trajectories over the scene footprint. Each trajectory corresponds to a measurement sequence used by the radio encoder.}
\label{fig:ue_trajectories}
\end{figure}
...
\begin{subfigure}[t]{0.32\linewidth}
  \centering
  \includegraphics[width=\linewidth]{figures/feature_violin_rt_features.png}
  \caption{RT feature distributions.}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.32\linewidth}
  \centering
  \includegraphics[width=\linewidth]{figures/feature_violin_phy_features.png}
  \caption{PHY feature distributions.}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.32\linewidth}
  \centering
  \includegraphics[width=\linewidth]{figures/feature_violin_mac_features.png}
  \caption{MAC feature distributions.}
\end{subfigure}
\caption{Violin plots of per-dimension feature distributions for the radio encoder inputs (RT/PHY/MAC).}
...
We also visualize the optional physics-only refinement step. Figure~\ref{fig:refinement_effect} shows how refinement moves the prediction along radio-map gradients and reduces the error for the shown sample. Figure~\ref{fig:error_overlay} overlays per-sample errors on the building footprint to highlight spatial error structure. Figure~\ref{fig:error_distribution} summarizes batch-level error statistics with a histogram, CDF, and box plot.

\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{figures/error_on_building_map.png}
\caption{Per-sample localization error overlaid on the building footprint. Color indicates Euclidean error magnitude.}
\label{fig:error_overlay}
\end{figure}
