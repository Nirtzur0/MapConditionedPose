...
\node[inputbox] (rtin) at (-5.5, 5.3) {RT Features\\{\tiny [B, L, 16] (Doppler stats)}};
\node[inputbox] (phyin) at (-3.5, 5.3) {PHY Features\\{\tiny [B, L, 8] (CQI/PMI/RI)}};
\node[inputbox] (macin) at (-1.5, 5.3) {MAC Features\\{\tiny [B, L, 6]}};

% Embedding/Projection layers
\node[embedbox] (cellemb) at (-5.5, 5.8) {Embed\\{\tiny →24}};
\node[embedbox] (beamemb) at (-3.5, 5.8) {Embed\\{\tiny →24}};
\node[embedbox] (timeemb) at (-1.5, 5.8) {SinPos\\{\tiny →24}};

\node[projbox] (rtproj) at (-5.5, 4.65) {Linear\\{\tiny →24}};
\node[projbox] (phyproj) at (-3.5, 4.65) {Linear\\{\tiny →24}};
\node[projbox] (macproj) at (-1.5, 4.65) {Linear\\{\tiny →24}};
...
\node[dimtext, below=0.05cm of concat] {{\tiny 6×24=144}};
...
% Linear projection
\node[projbox, minimum width=2.0cm, fill=embedgreen!40] (inputproj) at (-2.75, 2.9) {Linear(144→96)};
...
% Transformer Encoder
\node[transformer, minimum width=3.8cm] (radioenc) at (-2.75, 1.5) {\textbf{Transformer Encoder}\\{\scriptsize 2 layers, 4 heads, Pre-LN}};
\draw[arrow] (inputproj.south) -- ++(0, -0.25) -| (radioenc.north);
\draw[arrow] (cls.south) |- ([yshift=0.2cm]radioenc.north west);

% CLS output
\node[outputbox, fill=transformerorange!30, minimum width=2.2cm] (radiocls) at (-2.75, 0.3) {CLS Token\\{\tiny [B, 96]}};
...
% Patch embedding
\node[projbox, minimum width=2.4cm, fill=mapblue!30] (patchemb) at (5.5, 4.7) {Patch Embed\\{\tiny 32×32→128}};
\draw[arrow, mapblue!70] (mapconcat.south) -- (patchemb.north);

% Map Vision Transformer
\node[transformer, minimum width=3.6cm, fill=mapblue!25] (mapvit) at (5.5, 3.2) {\textbf{ViT Encoder (default)}\\{\scriptsize 2 layers, 4 heads}\\{\scriptsize E(2)-ViT optional (p4m)}};
\draw[arrow] (patchemb.south) -- (mapvit.north);

% Map spatial tokens output
\node[outputbox, fill=mapblue!30, minimum width=2.6cm] (maptokens) at (5.5, 1.65) {Spatial Tokens\\{\tiny [B, 64, 128]}};
\draw[arrow, mapblue!70] (mapvit.south) -- (maptokens.north);

% ============================================
% CENTER: CROSS-ATTENTION FUSION
% ============================================
\node[fusionbox, minimum width=4.2cm, minimum height=1.2cm] (fusion) at (1.5, -0.8) {\textbf{Cross-Attention Fusion}\\{\scriptsize Multi-Query (2 tokens)}\\{\scriptsize Q: Radio, K/V: Map}};
...
% Fused output
\node[outputbox, fill=fusionpurple!30, minimum width=2.4cm] (fused) at (1.5, -2.2) {Fused\\{\tiny [B, 192]}};
...
\begin{scope}[on background layer]
    \node[draw=inputblue!60, dashed, rounded corners=8pt, fit=(cellin)(radioenc)(radiocls), inner sep=8pt, fill=inputblue!5] {};
\end{scope}
...
\node[font=\sffamily\tiny, text=gray, align=left] at (-6.8, 4.0) {B = Batch\\L = Seq len\\K = Top-K (3)};

% Channel annotations for maps
\node[font=\sffamily\tiny, text=gray, right] at (7.8, 6.2) {Radio: PathGain, SNR,\\SINR, Throughput, BLER};
\node[font=\sffamily\tiny, text=gray, right] at (7.8, 5.4) {OSM: Height, Footprint,\\Roads, Terrain, Material};
