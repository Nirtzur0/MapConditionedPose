...
\node[cfrbox] (cfrin) at (-0.0, 5.3) {CFR\\{\tiny [B, 8, 64]}};
...
\node[projbox] (cfrenc) at (0.0, 4.65) {Conv1D\\{\tiny →128}};
...
\node[transformer, minimum width=3.6cm, fill=mapblue!25] (mapvit) at (5.5, 3.2) {\textbf{E(2)-ViT Encoder}\\{\scriptsize 6 layers, 8 heads}\\{\scriptsize p4m group (8 elements)}};
...
% ============================================
% Coarse Head
\node[headbox, fill=coarsered!25] (coarse) at (-1.5, -3.7) {\textbf{Coarse Head}\\{\scriptsize MLP → 32×32 grid}};
\draw[arrow] (fused.south) -- ++(0, -0.4) -| (coarse.north);

% Fine Head
\node[headbox, fill=fineyellow!40] (fine) at (4.5, -3.7) {\textbf{Fine Head}\\{\scriptsize Top-K refinement}};
\draw[arrow] (fused.south) -- ++(0, -0.4) -| (fine.north);

% Coarse outputs
\node[outputbox, fill=coarsered!20, minimum width=2.0cm] (heatmap) at (-2.8, -5.1) {Heatmap\\{\tiny [B, 32, 32]}};
\node[outputbox, fill=coarsered!20, minimum width=1.8cm] (topk) at (-0.3, -5.1) {Top-K\\{\tiny [B, K]}};
\draw[arrow, coarsered!70] (coarse.south) -- ++(0, -0.3) -| (heatmap.north);
\draw[arrow, coarsered!70] (coarse.south) -- ++(0, -0.3) -| (topk.north);

% Connection from coarse to fine (top-k indices)
\draw[dasharrow, coarsered!60] (topk.east) -- ++(0.5, 0) |- ([yshift=-0.2cm]fine.west) node[pos=0.3, above, font=\tiny] {indices};

% Fine outputs
\node[outputbox, fill=fineyellow!30, minimum width=1.7cm] (offsets) at (3.3, -5.1) {Offsets\\{\tiny [B, K, 2]}};
\node[outputbox, fill=fineyellow!30, minimum width=1.7cm] (sigma) at (5.7, -5.1) {Uncertainty\\{\tiny [B, K, 2]}};
\draw[arrow, fineyellow!60!orange] (fine.south) -- ++(0, -0.3) -| (offsets.north);
\draw[arrow, fineyellow!60!orange] (fine.south) -- ++(0, -0.3) -| (sigma.north);

% ============================================
% FINAL OUTPUT
% ============================================
\node[outputbox, fill=outputteal!40, minimum width=3.5cm, minimum height=0.9cm, very thick] (finalpos) at (1.5, -6.5) {\textbf{GMM Posterior}\\$p(\mathbf{y}) = \sum_k \pi_k \mathcal{N}(\boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k)$};
\draw[arrow, thick, outputteal] (heatmap.south) |- (finalpos.west);
\draw[arrow, thick, outputteal] (offsets.south) |- (finalpos);
\draw[arrow, thick, outputteal] (sigma.south) |- (finalpos.east);
