...
\node[outputbox, fill=fineyellow!30, minimum width=1.7cm] (sigma) at (5.4, -5.8) {Uncertainty\\{\tiny [B, K, 2]}};
\draw[arrow, fineyellow!60!orange] (fine.south) -- ++(0, -0.4) -| (offsets.north);
\draw[arrow, fineyellow!60!orange] (fine.south) -- ++(0, -0.4) -| (sigma.north);

% ============================================
% FINAL OUTPUT
% ============================================
\node[outputbox, fill=outputteal!40, minimum width=3.2cm, minimum height=0.8cm, very thick] (finalpos) at (1.5, -7.4) {\textbf{GMM Posterior}\\$p(\mathbf{y}) = \sum_k \pi_k \mathcal{N}(\boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k)$};
\draw[arrow, thick, outputteal] (heatmap.south) |- ++(0, -0.3) -| (finalpos.west);
\draw[arrow, thick, outputteal] (offsets.south) |- ++(0, -0.3) -| (finalpos);
\draw[arrow, thick, outputteal] (sigma.south) |- ++(0, -0.3) -| (finalpos.east);
