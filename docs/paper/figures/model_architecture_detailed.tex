...
% Transformer
\node[transformer] (radioenc) at (-2.5, 1.0) {\textbf{Transformer}\\{\scriptsize 2L, 4H, Pre-LN}};
...
% ViT
\node[transformer, fill=cyan!20] (mapvit) at (3.65, 1.7) {\textbf{ViT (default)}\\{\scriptsize 2L, 4H}\\{\scriptsize E(2)-ViT optional}};
...
% ============================================
\node[fusionbox] (fusion) at (0.5, -1.3) {\textbf{Cross-Attention}\\{\scriptsize 2 query tokens, 4 heads}};
...
% Fine Head  
\node[headbox, fill=yellow!25, font=\scriptsize] (fine) at (2.8, -3.8) {\textbf{Fine Head}\\{\tiny Top-K=3 Re-ranking}};
...
\node[outputbox, fill=red!20, font=\scriptsize, minimum width=1.5cm] (topk) at (-0.8, -5.0) {Top-K\\{\tiny [3]}};
...
% Fine outputs
\node[outputbox, fill=yellow!25, font=\scriptsize, minimum width=1.3cm] (offsets) at (2.0, -5.0) {$\boldsymbol{\Delta}_k$\\{\tiny [K,2]}};
\node[outputbox, fill=yellow!25, font=\scriptsize, minimum width=1.3cm] (scores) at (3.6, -5.0) {$s_k$\\{\tiny [K]}};
\draw[arrow, yellow!50!orange] (fine.south) -- ++(0, -0.3) -| (offsets.north);
\draw[arrow, yellow!50!orange] (fine.south) -- ++(0, -0.3) -| (scores.north);

% ============================================
% FINAL OUTPUT
% ============================================
\node[outputbox, fill=teal!30, font=\scriptsize, minimum width=3.6cm, thick] (finalpos) at (0.5, -6.2) {$\alpha_k = \mathrm{softmax}(\log \pi_k + s_k)$\\$\hat{\mathbf{y}} = \sum_{k=1}^K \alpha_k\,(\mathbf{c}_k + \boldsymbol{\Delta}_k)$};
\draw[arrow, thick, teal!60] (heatmap.south) |- ++(0, -0.3) -| (finalpos.west);
\draw[arrow, thick, teal!60] (offsets.south) |- ++(0, -0.3) -| (finalpos);
\draw[arrow, thick, teal!60] (scores.south) |- ++(0, -0.3) -| (finalpos.east);
