...
\pagestyle{fancy}
\fancyhf{}
\lhead{Confidential Research Draft}
...
\begin{align}
  \delta_t &= \tau_t-\tau_1,\qquad \bar\delta_t=\frac{\delta_t}{\max_s \delta_s + \varepsilon}, \nonumber \\
  \psi(\tau_t;\bm\tau)&=\mathrm{PE}(\bar\delta_t),
  \label{eq:time_norm}
\end{align}
...
\begin{align}
  \mathbf{z}_f
  &= \mathrm{MHA}\big(Q=\mathbf{z}_r W_Q,\;K=\mathbf{S} W_K,\;V=\mathbf{S} W_V\big) \nonumber \\
  &\quad \in \R^{d_f}.
  \label{eq:crossattn}
\end{align}
...
\begin{align}
  ...
  &= \sum_{c=1}^{G^2} p_\theta(C=c\mid \mathcal{X},\mathcal{M}) \times \nonumber \\
  &\quad p_\theta(\mathbf{y}\mid C=c,\mathcal{X},\mathcal{M}).
  \label{eq:hier_posterior}
\end{align}
The coarse head outputs logits over cells and a categorical distribution
\begin{align}
  \pi_c &= p_\theta(C=c\mid\mathcal{X},\mathcal{M}) = \softmax(\mathbf{u})_c, \nonumber \\
  \mathbf{u} &= \mathrm{MLP}_{\text{coarse}}(\mathbf{z}_f).
  \label{eq:coarse_pi}
\end{align}
...
\begin{align}
  \hat\pi_k &= \frac{\pi_{c_k}}{\sum_{j=1}^K \pi_{c_j}}, \nonumber \\
  k &= 1,\dots,K.
  \label{eq:topk_pi}
\end{align}
...
\begin{align}
  ...
  &= \sum_{k=1}^K \hat\pi_k \times \nonumber \\
  &\quad \mathcal{N}\!\left(\mathbf{y};\mathbf{c}_{c_k}+\bm\mu_{c_k},\diag(\bm\sigma_{c_k}^2)\right).
...
\begin{align}
  ...
  \Cov[\mathbf{y}]
  &= \sum_{k=1}^K \hat\pi_k \Big(\bm\Sigma_k + \nonumber \\
  &\quad (\bm m_k-\E[\mathbf{y}])(\bm m_k-\E[\mathbf{y}])^\top\Big).
...
\begin{align}
  \mathcal{L}_{\text{fine}}(\theta) &= -\log p_{\theta,K}(\mathbf{y}^\star\mid \mathcal{X},\mathcal{M}) \nonumber \\
  &= -\LogSumExp_{k=1}^K \Big( \log \hat\pi_k + \nonumber \\
  &\quad \log \mathcal{N}(\mathbf{y}^\star;\bm m_k,\bm\Sigma_k) \Big).
...
\begin{align}
  ...
  &\propto \sum_{j=1}^{C_{\text{phys}}} \frac{(m_j-r_j(\mathbf{y}))^2}{\sigma_{\text{phys},j}^2} \nonumber \\
...
\begin{align}
  \mathcal{E}(\mathbf{y}) &= \alpha\Big(-\log p_{\theta,K}(\mathbf{y}\mid\mathcal{X},\mathcal{M})\Big) \nonumber \\
  &\quad + \beta\,\mathcal{L}_{\text{phys}}(\mathbf{y}),
