...
def plot_ue_trajectories(
    ...
    resolved_coord_mode = coord_mode
    map_ready = False
    map_extent = None
    footprint = None
    ...
    with dataset._env.begin() as txn:
        for color, seq_id in zip(colors, chosen):
            ...
            if resolved_coord_mode == "auto":
                resolved_coord_mode = "global" if _points_look_global(points, bbox) else "scene"
            points = _coerce_points(points, bbox, resolved_coord_mode)

            if show_map and not map_ready and bbox is not None and scene_id is not None:
                try:
                    radio_map, osm_map = dataset._get_scene_maps(scene_id)
                    osm_np = osm_map.cpu().numpy() if torch.is_tensor(osm_map) else np.asarray(osm_map)
                    footprint = _prettify_footprint(osm_np)
                except Exception:
                    footprint = None

                if footprint is not None:
                    if resolved_coord_mode == "global":
                        map_extent = [bbox["x_min"], bbox["x_max"], bbox["y_min"], bbox["y_max"]]
                    else:
                        width = bbox["x_max"] - bbox["x_min"]
                        height = bbox["y_max"] - bbox["y_min"]
                        map_extent = [0.0, float(width), 0.0, float(height)]
                    ax.imshow(
                        footprint,
                        cmap="Greys",
                        origin="lower",
    ...
    coord_label = "Global" if resolved_coord_mode == "global" else "Scene-local"
    ax.set_title(f"UE Trajectories (n={num_pick}) | {coord_label}")
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_aspect("equal", adjustable="box")
...
def visualize_sionna_3d_scene(
    ...
    try:
        ...
        try:
            if verbose:
                print("Rendering Sionna scene with radio overlay...")
            fig = scene.render(
                camera=cam,
                radio_map=radio_map,
                rm_metric="path_gain",
            ...
            print(f"Sionna render failed: {exc}")
        ...
        if show_heatmap:
            ...
            if verbose:
                print("Extracting radio map for 2D heatmap...")
            ...
            ax.scatter([ue_x], [ue_y], c='lime', s=150, marker='*', edgecolors='black', linewidth=1.5, zorder=10)
            ax.scatter([cx], [cy], c='red', s=100, marker='^', edgecolors='black', linewidth=1.5, zorder=10)
            ax.set_xlabel('X (meters)')
            ax.set_ylabel('Y (meters)')
...
def visualize_feature_histograms(batch, max_dims=12, use_mask=True):
    """Plot histograms for RadioEncoder input features (RT/PHY/MAC only)."""
    ...
    def _plot_grid(data, title, max_dims_local):
        ...
        for i in range(dims):
            ...
            ax.set_title(f"{title} dim {i}", fontsize=12)
    ...
    for key in ('rt_features', 'phy_features', 'mac_features'):
        if key in measurements:
            _plot_grid(measurements[key].cpu().numpy(), key, max_dims)
...
def visualize_feature_violinplots(*args, **kwargs):
    """Deprecated: violin plots removed in favor of histograms only."""
    raise RuntimeError("visualize_feature_violinplots has been removed. Use visualize_feature_histograms instead.")
